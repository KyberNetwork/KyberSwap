...
const TokenView = (props) => {
  ...
  if (props.token){
    if(props.type == 'transfer'){
      return (
        <label>Select Token
          <div onClick={props.onSelected} class="token-select" data-open="transfer-from-token-modal"><img src={props.token.icon}/><span class="name">{props.token.name}</span></div>
        </label>
      )
